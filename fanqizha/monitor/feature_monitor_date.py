...
# -*- encoding: utf-8 -*-
"""
线下，人工查看某天用户的特征，包括：纪录某天用户每个特征平均值；纪录，某天用户命中规则数c卡和闪银分比较
"""
...
class FeatureMonitor(object):
    ...
    def get_mg_conn(self,mc):
        ...
        return mdb[mc]
    def getwecashAndselfscore(self,dl1,dl2):
        ...
        #            'warehouse.risk_credit_chain rcc join warehouse.risk_user_merchant rum on' \
        #            ' rcc.userId=rum.userId and rum.merchantCode="1001" join ' \
        #            'sudaibear.uc_users uu on rum.merchantUserId+0=uu.id and ' \
        #            'uu.userType=2 LEFT JOIN warehouse.risk_wecash_contact rwc ON ' \
        #            'rcc.userId = rwc.userId LEFT JOIN warehouse.risk_fraud_score fs ON ' \
        #            'rcc.userId = fs.userId LEFT JOIN warehouse bb ON ' \
        #            'rum.merchantUserId = bb.userId LEFT JOIN ( SELECT t.userid AS userid,t.chainid, ' \
        #            'count(*) AS rulenum FROM warehouse.risk_auth_warnrule t WHERE t.ruleCode ' \
        #            '<> "fraud_score" and t.type=0 GROUP BY t.userId,t.chainid ) wcru on ' \
        #            'rcc.userid=wcru.userid and rcc.id=wcru.chainid where rcc.pattern like "payday_basic%" and ' \
        #            'rcc.createDate>= "'+dl1+'" and rcc.createDate<"'+dl2+'"'
        sql='select distinct rum.userid as "userid",uu.mobile as "phn", ' \
            'rcc.createDate as "time",rwc.contactScore AS "wecashscore",' \
            'fs.score AS "fraudScore",CASE WHEN rcc.`status` LIKE "refuse%" ' \
            'THEN "-3" WHEN rcc.`status` LIKE "pass%" AND bb.id IS NULL THEN "-2" ' \
        ...
        if n != 0:
            ...
            for fli in content:
                if fli[1] in userinfdic:
                    print fli[5],userinfdic[fli[1]]
                    if userinfdic[fli[1]][5] is None or fli[5] is None:
                        continue
                    try:
    ...
    def ruleDistribution(self,userlist,dl1):
        nowdate=datetime.datetime.now()
        yestoday=nowdate
        # out_file='/tmp/modelmonitor/ruledistribution/'+yestoday.strftime('%Y-%m-%d')
        out_file = '/home/xuyonglong/ruledis/modelmonitor/ruledistribution/'+dl1
        wfp=open(out_file,'w')
        ...
        ruledis(aucdfc)
    def basefeatureDistribution(self,phnlist,dl1):
        ...
        # print desdf['mean']
        # print desdf['meangap']
        # desdf.to_csv('/tmp/modelmonitor/basefeature/'+yestoday+'.csv',index=False)
        desdf.to_csv('/home/xuyonglong/ruledis/modelmonitor/basefeature/'+dl1+'.csv', index=False)
if __name__ == '__main__':
    fm=FeatureMonitor()
    datelist=['2017-09-06','2017-09-07']
    for i in range(0,len(datelist)-1):
        phnlist, userlist=fm.getwecashAndselfscore(datelist[i],datelist[i+1])
        # print len(phnlist),len(userlist)
        fm.ruleDistribution(userlist,datelist[i])
        fm.basefeatureDistribution(phnlist,datelist[i])
