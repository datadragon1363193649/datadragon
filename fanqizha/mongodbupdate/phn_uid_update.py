...
class PutData(object):
    ...
    def init_online_mg_conn(self):
        self.online_mgconn = pm.MongoClient([ dconf.online_mg_host2],
                                      maxPoolSize=10)
        self.online_mgconn[dconf.online_mg_db].authenticate(dconf.online_mg_uname, dconf.online_mg_passwd)

    def get_mg_conn(self):
        mc = self.mgcollection
        mdb = self.mgconn[self.mgdb]
        return mdb[mc]
    def get_mg_conn_base(self):
        mc = self.mgbasefeature
        mdb = self.mgconn[self.mgdb]
        return mdb[mc]
    def get_online_mg_conn_base(self):
        mc = dconf.online_mg_base_feature
        mdb = self.online_mgconn[dconf.online_mg_db]
        return mdb[mc]
    ...
    def put_pass_data_batch(self):
        ...
        with open(s_file, 'r') as fp:
            ...
            if i % 1000 != 0:
                self.put_data_tag_batch(phn, tag)
    def phn_Traversal(self,mysqlcursor,base_mg_conn,phn):
        re = base_mg_conn.find({'_id': {'$in': phn}})
        # re = base_mg_conn.find({'_id': '18612395386'})
        rsl = list(re)
        # print rsl
        args = ','.join(['%s'] * len(phn))
        # print phn
        sql = 'select id,mobile from risk_user where mobile in (%s)' % args
        try:
            self.mysqlconn.ping()
        except:
            self.init_mysql_conn()
        ...
        pid_data=()
        try:
            mysqlcursor.execute(sql, phn)
            pid_data = mysqlcursor.fetchall()
            # print data
        except:
            pass
        # print pid_data
        ...
        if len(pid_data)>0:
            for di in pid_data:
                try:
                    phndic[di[1]]=di[0]
                    uidlist.append(str(di[0]))
                except:
                    pass
            # print uidlist
            re = base_mg_conn.find({'_id': {'$in': uidlist}})
            res = []
            for i in re:
                res.append(i['_id'])
            # print res
            ...
            for rl in rsl:
                if rl['_id'] in phndic:
                    # print rl['_id']
                    rl['_id']=str(phndic[rl['_id']])
                    # print rl['_id']
                    if rl['_id'] in res:
                        calldic = {}
                        for call in rl['call']:
                            calldic['call.' + call] = rl['call'][call]
                        base_mg_conn.update({'_id': rl['_id']}, {'$set': calldic})
                        continue
                    # print rl
                    uiddic.append(rl)
            if len(uiddic)>0:
                base_mg_conn.insert_many(uiddic)

    def phn_to_uid(self):
        mysqlcursor = self.mysqlconn.cursor()
        base_mg_conn = self.get_mg_conn_base()
        # s_file =  'D:/xuyl/data/phn.txt'
        s_file=apath+'/data/phn.txt'
        phn = []
        i = 0
        with open(s_file, 'r') as fp:
            for line in fp:
                ...
                i += 1
                if i % 1000 == 0:
                    self.phn_Traversal(mysqlcursor,base_mg_conn,phn)
                    time.sleep(1)
                    phn = []
        if i % 1000 != 0:
            # print phn
        ...
        # blacksql = 'select id from risk_user where mobile= '+mobile
        # self.put_data_tag(mysqlcursor, blacksql, 'is_black')
    def phn_Traversal_online(self,mysqlcursor,base_mg_conn,online_mg_conn,uid):

        # # print rsl
        args = ','.join(['%s'] * len(uid))
        # print phn
        sql = 'select id,mobile from risk_user where id in (%s)' % args
        try:
            self.mysqlconn.ping()
        except:
            self.init_mysql_conn()
        ...
        if len(pid_data)>0:
            for di in pid_data:
                try:
                    ...
                    pass
            # print uidlist
            re = online_mg_conn.find({'_id': {'$in': phnlist}})
            # # re = base_mg_conn.find({'_id': '18612395386'})
            rsl = list(re)
            re1 = base_mg_conn.find({'_id': {'$in': uidlist}})
            res = []
            for i in re1:
                res.append(i['_id'])
            # print res
            ...
            for rl in rsl:
                if rl['_id'] in phndic:
                    ...
                    if rl['_id'] in res:
                        ...
                        base_mg_conn.insert({'_id':  rl['_id']})
                        calldic = {}
                        for call in rl['call']:
                            calldic['call.' + call] = rl['call'][call]
                        base_mg_conn.update({'_id': rl['_id']}, {'$set': calldic})
    ...
    def phn_to_uid_online(self):
        mysqlcursor = self.mysqlconn.cursor()
        base_mg_conn = self.get_mg_conn_base()
        online_mg_conn=self.get_online_mg_conn_base()
        # s_file =  'D:/xuyl/data/phn.txt'
        s_file=apath+'/data/uid.txt'
        uid = []
        i = 0
        with open(s_file, 'r') as fp:
            for line in fp:
                linelist = line.strip().split(',')
                uid.append(linelist[0])
                i += 1
                if i % 1000 == 0:
                    self.phn_Traversal_online(mysqlcursor, base_mg_conn,online_mg_conn, uid)
                    time.sleep(1)
                    uid = []
        if i % 1000 != 0:
            # print uid
