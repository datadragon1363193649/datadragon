...
class AllfeatureFind(object):
    ...
    def __init__(self):
        ...
        self.operator_dic = dconf.operator_dic
    ...
    def get_td_fea(self,phn):
        session = self.Session()
        try:
            re = session.query(RiskUser.id).filter(RiskUser.mobile == phn).first()
        except Exception, e:
            JKLOG.Error([('e', 'get user_risk id failue'), ('exp type', str(e))])
            # raise HttpJsonError(602, 'Get Info From DB Error')
            return [-1,-1,-1,-1]
        session.close()
        namedic={}
        try:
            callmonconn = self.get_mg_connlbs(dconf.mg_user_td_detail)
            # print re.id
            rs = callmonconn.find({'_id':str(re.id)})
            if rs.count() == 0:
                return [-1, -1, -1, -1]
            else:
                rsl = list(rs)
                # print rsl[0]
                if 'viewDetail' in rsl[0]:
                    # print rsl[0]['viewDetail']
                    viewjson=json.loads(rsl[0]['viewDetail'])
                    # print viewjson[0]
                    if 'rules' in viewjson[0]:
                        ...
                        for rule in viewrule:
                            if 'rule_name' in rule:
                                if rule['rule_name'] in self.tongdun_feature_name:

                                    if rule['rule_name'] ==u'3个月内身份证关联多个申请信息':
                                    ...
                                    elif rule['rule_name'] in [u'身份证命中低风险关注名单',u'身份证命中中风险关注名单']:
                ...
                if len(namedic) > 0:
                    featurelist=[]
                    for ni in self.tongdun_feature_name:
                        if ni in namedic:
                            featurelist.append(namedic[ni])
                        else:
            ...
            JKLOG.Error([('e', 'get tongdun detail failue'), ('exp type', str(e))])
            return [-1, -1, -1, -1]
    def get_differ_num_fea(self,cinfo):
        first_call_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        begin_call_time = datetime.datetime.strptime(first_call_time, "%Y-%m-%d %H:%M:%S")
        left_call_time=begin_call_time
        anchor1 = self.before_some_days(first_call_time, 30)
        anchor2 = self.before_some_days(first_call_time, 90)
        ...
        call_contacts=cinfo['contacts']
        operation_phn=''
        if 'operation_type' in call_details:
            if call_details['operation_type'] in [u'移动',u'联通',u'电信']:
                operation_phn = self.operator_dic[call_details['operation_type']]
            # elif call_details['operation_type'] == u'联通':
            #     operation_phn = self.operator_dic[call_details['operation_type']]
            # elif call_details['operation_type'] == u'电信':
            #     operation_phn = self.operator_dic[call_details['operation_type']]
        # print operation_phn
        try:
            first_call_time = call_details['call_info'][0]['start_time']
            ...
            JKLOG.Error([('data', 'call_info error'), ('error', str(e))])
        # print anchor1
        detail_phn_dic={}
        max_days = -1
        passivity_num=0
        ...
        for c in call_details['call_info']:
            try:
                ...
                if st == '':
                    continue
                detail_phn_dic[c['phone_num']] = 0
                if st > anchor1:
                    right_call_time = datetime.datetime.strptime(st, "%Y-%m-%d %H:%M:%S")
                    days = (left_call_time - right_call_time).days
                    # print stleft,stbegin, stright,days,phn
                    if days < 1:
                        left_call_time = right_call_time
                    else:
                        day_sum = (begin_call_time - left_call_time).days
                        # print begin_call_time, left_call_time,day_sum
                        if day_sum > max_days:
                            max_days = day_sum
                        left_call_time = right_call_time
                        begin_call_time = right_call_time

                if st>anchor2 and operation_phn!='':
                    call_phn = c['phone_num']
                    if operation_phn in call_phn:
                        # phnindex=callphn
                        # print operation_phn,call_phn
                        if call_phn.index(operation_phn)==0 or (call_phn.index(operation_phn)
                                                                    +len(operation_phn))==len(call_phn):
                            # print call_phn.index(operation_phn)
                ...
                JKLOG.Error([('data', 'call_details error'), ('error', str(e))])

        day_sum = (begin_call_time - left_call_time).days
        # print 'end', begin_call_time, left_call_time,day_sum
        if day_sum > max_days:
            max_days = day_sum
        max_call_contacts_ratio = max_days * 1.0 / (len(call_contacts) + 1)
        max_call_details_ratio = max_days * 1.0 / (len(detail_phn_dic) + 1)
        # print max_days,len(call_contacts),len(detail_phn_dic)
        operator_differ_num = accord_num - passivity_num
        return [max_call_contacts_ratio,max_call_details_ratio,operator_differ_num]
    def get_maxcom_differ_fea(self, phn):
        # print phn
        maxcom_differ_fea = [-1, -1, -1]
        # stype = -1
        # phn = '15774913554'
        # sessionsudai = self.Sessionsudai()
        c = self.get_mg_conn(dconf.mg_collection)
        ...
        if not cinfo:
            JKLOG.Info([('find', 'mongodb'), ('phone', phn), ('info', 'no phn')])
            # phnnumfea.append(-1)
            return maxcom_differ_fea
        if 'call_details' in cinfo and 'contacts' in cinfo:
            # maxnocom = self.max_no_communicate(cinfo['call_details'])

            maxcom_differ_fea = self.get_differ_num_fea(cinfo)
        else:
            maxcom_differ_fea = [-1, -1, -1]

        # print maxcom_differ_fea
        return maxcom_differ_fea
    ...
    def get_all_feature(self):
        ...
        for phn in userdic:
            ...
            if rs.count() > 0:
                if 'call' in rs:
                    if 'max_call_contacts_ratio' in rs['call']:
                        continue
            ...
            #                                   }
            #                          })
            maxcom_differ_fea=self.get_maxcom_differ_fea(phn)
            cf.update({'_id': phn}, {'$set': {'call.max_call_contacts_ratio': maxcom_differ_fea[0],
                                              'call.max_call_details_ratio': maxcom_differ_fea[1],
                                              'call.operator_differ_num': maxcom_differ_fea[2],
                                              }
                                     })
            # daystr=[]
